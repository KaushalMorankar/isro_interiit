...
# Path to the lunar albedo map
albedo_map_path = 'lunar_albedo_map.tif'  # Adjust to your actual file path
...
def plot_abundance_heatmap(max_val):
    ...
    # Load the lunar albedo map if available
    try:
        with rasterio.open(albedo_map_path) as lunar_albedo:
            show(lunar_albedo, ax=ax, cmap='gray')
            width = lunar_albedo.width
            height = lunar_albedo.height
    except FileNotFoundError:
        print("Lunar albedo map file not found. Proceeding with only XRF coverage regions.")
    ...
    ax.set_title(f"XRF Coverage Regions - Mg/Al Ratio Heatmap")
...
while(True):
    ...
    max_val = int(input("Enter maximum value for colour scale (-1 to exit): "))
