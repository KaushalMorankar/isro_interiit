...
def plot_heatmap(max_val):
    ...
    cmap = plt.cm.jet
    # Set the scale
    norm = mcolors.Normalize(vmin=0.5, vmax=max_val)
    ...
    for _, row in xrf_data.iterrows():
        ...
        for poly_corners in split_polygons:
            if len(poly_corners) > 2:
                ...
                abundance = (row[f'Abundance_Mg (%)']/1.2)/row[f'Abundance_Si (%)']
                if abundance>1.2:
    ...
    ax.set_title(f"XRF Coverage Regions - Mg/Si Ratio Heatmap")
