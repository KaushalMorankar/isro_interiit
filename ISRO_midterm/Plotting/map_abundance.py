...
def plot_heatmap(element, max_val, scale_factor, abundance_threshold):
    ...
    ax.set_title(f"XRF Coverage Regions - {element} Abundance Heatmap")
    plt.xlabel("Longitude (degrees)")
    plt.ylabel("Latitude (degrees)")
    plt.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, label=f'Abundance_{element} (%)')
    plt.show()
